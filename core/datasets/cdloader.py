...
class HeteCDReader(Dataset):
    """
    HeteCD / XiongAn dataset reader.
    ...
    A: Optical RGB, 3 channels
    B: SAR, 3 channels in XiongAn paper: HH, VV, HV
       如果你的 B 是伪彩色 SAR PNG，也按 3 通道读取。
    label: binary change mask
    """

    def __init__(self, path_root="./dataset/", mode="train", img_size=512):
        super(HeteCDReader, self).__init__()
        ...
        self.path_root = os.path.join(path_root, mode)
        self.A_dir = os.path.join(self.path_root, "A")
        self.B_dir = os.path.join(self.path_root, "B")
        self.label_dir = os.path.join(self.path_root, "label")
        ...
        A_files = self._list_image_files(self.A_dir)
        B_map = self._build_stem_map(self.B_dir)
        L_map = self._build_stem_map(self.label_dir)

        for a_file in A_files:
            stem = os.path.splitext(a_file)[0]

            if stem not in B_map:
                raise FileNotFoundError(
                    f"Cannot find SAR image for A/{a_file}. "
                    f"Expected same stem in {self.B_dir}."
                )

            if stem not in L_map:
                raise FileNotFoundError(
                    f"Cannot find label for A/{a_file}. "
                    f"Expected same stem in {self.label_dir}."
                )

            self.opt_images.append(os.path.join(self.A_dir, a_file))
            self.sar_images.append(os.path.join(self.B_dir, B_map[stem]))
            self.gt_images.append(os.path.join(self.label_dir, L_map[stem]))
            self.file_name.append(stem + ".png")

        self.data_num = len(self.opt_images)

        print(f"[HeteCDReader] {mode}: {self.data_num} samples")
        print(f"[HeteCDReader] A dir: {self.A_dir}")
        print(f"[HeteCDReader] B dir: {self.B_dir}")
        print(f"[HeteCDReader] label dir: {self.label_dir}")

    def __getitem__(self, index):
        A_path = self.opt_images[index]
        B_path = self.sar_images[index]
        L_path = self.gt_images[index]

        opt_img = self._read_3ch(A_path)
        sar_img = self._read_3ch(B_path)
        label = self._read_label(L_path)

        opt_img, sar_img, label = self._resize(opt_img, sar_img, label)

        if self.aug:
            opt_img, sar_img, label = self._augment(opt_img, sar_img, label)

        opt_img = self._normalize_01(opt_img)
        sar_img = self._normalize_01(sar_img)

        opt_img = np.transpose(opt_img, (2, 0, 1)).astype("float32")
        sar_img = np.transpose(sar_img, (2, 0, 1)).astype("float32")

        label = self._to_one_hot(label).astype("float32")

        opt_img = paddle.to_tensor(opt_img)
        sar_img = paddle.to_tensor(sar_img)
        label = paddle.to_tensor(label)

        image = paddle.concat([opt_img, sar_img], axis=0)  # [6, H, W]

        if self.aug:
            return {"img": image, "label": label}
        else:
            return {"img": image, "label": label, "name": self.file_name[index]}
    ...
    def _build_stem_map(folder):
        files = HeteCDReader._list_image_files(folder)
        stem_map = {}
    ...
    def _resize(self, opt_img, sar_img, label):
        h, w = opt_img.shape[:2]
        ...
        if opt_img.ndim == 2:
            opt_img = np.stack([opt_img, opt_img, opt_img], axis=-1)

        if sar_img.ndim == 2:
            sar_img = np.stack([sar_img, sar_img, sar_img], axis=-1)

        return opt_img, sar_img, label

    @staticmethod
    def _augment(opt_img, sar_img, label):
        opt_img, sar_img, label = imutils.random_fliplr(opt_img, sar_img, label)
        opt_img, sar_img, label = imutils.random_flipud(opt_img, sar_img, label)
        opt_img, sar_img, label = imutils.random_rot(opt_img, sar_img, label)
        return opt_img, sar_img, label
    ...
    def _normalize_01(img):
        ...
        # XiongAn paper uses pixel values divided by 255.
        if img.max() > 1.5:
            img = img / 255.0
    ...
    def _to_one_hot(label):
        change = (label > 0).astype("int64")
        background = 1 - change
        one_hot = np.stack([background, change], axis=0)
        return one_hot
